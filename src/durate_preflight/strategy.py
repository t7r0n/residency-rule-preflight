...
COMPANY = 'Durate'
REPO = 'durate-preflight'
PROJECT_TERMS = ['durate', 'marketing', 'leans', 'plain', 'english', 'configuration', 'profile', 'residency']
PROJECT_METRICS = ['durate_coverage', 'marketing_risk', 'leans_precision', 'plain_latency']
PROJECT_FAILURES = ['durate_drift', 'marketing_gap', 'leans_misroute', 'plain_blindspot']
PROJECT_ARCHETYPES = [{'name': 'durate evidence replay', 'trigger': 'durate signal changes while marketing context is stale', 'expected': 'block release until cited evidence is regenerated'}, {'name': 'marketing boundary probe', 'trigger': 'marketing handoff crosses a policy or trust boundary', 'expected': 'route to reviewer with evidence packet'}, {'name': 'leans regression harness', 'trigger': 'leans behavior regresses against the last accepted fixture', 'expected': 'open a regression issue with trace and benchmark delta'}, {'name': 'plain operator packet', 'trigger': 'plain output needs a human-readable audit packet', 'expected': 'accept only if decision claims cite fixture evidence'}]
PROJECT_DIRECTION = 'A compliance pre flight layer for Durate that converts plain English rules into typed constraints, statically proves every published schedule against the ACGME Common Program Requirements + specialty supplements, and produces an auditor ready PDF showing exactly which rule check passed which clause.'
...
HERO_TITLE = 'Durate Evidence Triage Board'
